...
void ui_Artificial_Horizon_Page_screen_init(void)
{
    ...
    // Using ui_Label5 as an example for the main title, adjust if it's ui_ArtificialHorizonPageLabel
    ...
    // This object (uic_ArtificialHorizon) should be created by SquareLine Studio.
    // If it's not, you'd create it here:
    // uic_ArtificialHorizon = lv_obj_create(ui_Artificial_Horizon_Page);
    // Assuming it is already declared and will be created by SLS's ui_init() or similar logic.
    // If you need to create it because SLS only declared it as extern:
    if (!uic_ArtificialHorizon) { // Check if it was created by SLS, if not, create it.
        uic_ArtificialHorizon = lv_obj_create(ui_Artificial_Horizon_Page);
    }

    // Set size and position for the AHRS container
    // Example: 160x120 pixels, centered horizontally, specific Y offset
    ...
    lv_coord_t container_w = lv_obj_get_width(uic_ArtificialHorizon);
    lv_coord_t container_h = lv_obj_get_height(uic_ArtificialHorizon);
    //create_artificial_horizon_widget(uic_ArtificialHorizon, &ahrs_data, container_w, container_h); // Width and height passed here
    create_artificial_horizon_widget(uic_ArtificialHorizon, &ahrs_data, 160, 120);

    // Create and start the timer for AHRS updates
    if (ahrs_update_timer) { // Delete old timer if it exists (e.g., if screen re-initialized)
    ...
    // Add event callback for Home button (ui_event_Page2HomeBTN should be defined in ui_events.c)
    if(ui_event_Page2HomeBTN) { // Check if function pointer is not NULL
    ...
    if(ui_event_Page2NextBTN2) { // This should be the event handler for the NEXT button
