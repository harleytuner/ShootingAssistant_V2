...
void ui_Bubble_Level_Page_screen_init(void)
{
    ...
    // Initialize colors for the bubble level elements
    color_page_bg_bubble = lv_obj_get_style_bg_color(ui_Bubble_Level_Page, LV_PART_MAIN);
    ...
    color_text_roll_label = lv_color_hex(0xFFFFFF); // Assuming dark page bg for white text
    ...
    // Y: -10 (canvas target offset) - half canvas height - some spacing (e.g., 15px)
    // The main label is at -110, canvas is at -10.
    // Let's place roll angle label at Y = -10 (canvas Y) - LEVEL_CANVAS_HEIGHT/2 - 15
...
void ui_Bubble_Level_Page_screen_destroy(void)
{
    ...
    if(ui_Bubble_Level_Page) { // Check if the screen object exists
...
static void draw_bubble_level_on_canvas(float roll_angle_deg) {
    ...
    if (bubble_travel_range_px < 0) bubble_travel_range_px = 0; // Prevent negative range
    ...
    if (abs_actual_angle <= 3.0f) {  //change here to modify bubble colors
