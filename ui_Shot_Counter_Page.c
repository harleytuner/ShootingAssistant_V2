...
static int current_set_shot_count = DEFAULT_INITIAL_SHOT_COUNT; 
static int shots_remaining_count = DEFAULT_INITIAL_SHOT_COUNT;  
...
void ui_shot_counter_reset_all_to_default(void) { // No longer takes an argument
    current_set_shot_count = DEFAULT_INITIAL_SHOT_COUNT; // Uses the #define from its own header
    shots_remaining_count = DEFAULT_INITIAL_SHOT_COUNT;  // Uses the #define from its own header
...
static void update_shot_display_label(void)
{
    if (ui_ShotDisplayValueLabel) { 
        char buf[4]; 
        lv_snprintf(buf, sizeof(buf), "%d", shots_remaining_count);
